...
const shallow_cmp = (node1, node2) => {
    return node1.TYPE === node2.TYPE && node1.shallow_cmp(node2);
};

class Marker extends AST_Node {
    constructor(marker_id) {
        super({});
        this.flags = marker_id;
    }
    shallow_cmp(other) {
        return (other instanceof Marker) && other.flags === this.flags;
    }
}
...
export const equivalent_to = (tree1, tree2) => {
    ...
    while (walk_1_state.length && walk_2_state.length) {
        ...
        walk_1_state.push(new Marker(marker_id));
        walk_2_state.push(new Marker(marker_id));
        marker_id++;
    }
