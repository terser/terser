        ...
        walk_parent(node_with_body, (node, info) => {
            ...
            if (node instanceof AST_Defun) {
                if (info.parent() !== node_with_body) {
                    throw new NOPE();  // Defun in a block scope
                }
                ...
                if (block_scope) {
                    for (const definition of node.definitions) {
                        if (definition.name instanceof AST_Symbol) {
                            ...
                            throw new NOPE(); // TODO: destructuring
                        }
                    }
...
export class Binding {
    ...
    write(type, { is_definition = false }) {
        ...
        if (this.type == null) {
            ...
            if (is_tdz_violation) throw new NOPE();
            ...
            if (is_const_violation) throw new NOPE();
...
export const UnknownBinding = (() => {
    const UnknownBinding = new Binding("unknown", UnknownType, "unknown");
    UnknownBinding.changed = () => UnknownBinding;
    UnknownBinding.OR = () => UnknownBinding;
    UnknownBinding.read = () => UnknownBinding;
    UnknownBinding.write = () => UnknownBinding;
    UnknownBinding[Symbol.for("nodejs.util.inspect.custom")] = () => "UnknownBinding {}";

    return Object.freeze(UnknownBinding);
})();
...
export class World {
    ...
    define(name, type, binding_type = "var") {
        ...
        // assigning global
        if (this.parent_scope == null && !this.module) return UnknownType;
    ...
    decrement_binding(name, side = "reads") {
        if (!this.variables.has(name)) {
            return this.parent_scope && this.parent_scope.decrement_binding(name);
        } else {
    ...
    write_variable(name, type, {is_definition = true, track = this.track_rw} = {}) {
        ...
        if (!binding) {
            if (!this.parent_scope) throw new NOPE();
    ...
    read_binding(name, { track = this.track_rw, optional = false } = {}) {
        if (!this.variables.has(name)) {
            if (!this.parent_scope) {
                if (!optional) throw new NOPE();
                return UnknownBinding;
            }
            return this.parent_scope.read_binding(name, { track, optional });
        } else {
            let binding = this.variables.get(name);
    ...
    read_variable(name, opts) {
        return this.read_binding(name, opts).type;
    }
    read_variable_ambient(name, opts) {
        return this.read_binding(name, opts).ambient_type;
    }
    ...
    join(forked) {
        if (forked.parent_reality !== this) throw new NOPE();
...
export class NOPE extends Error {
    constructor(message = "") {
        super(message || "flow analysis impossible");
    }

    get is_nope() { return true; }
}
