...
function minify(files, options) {
    ...
    try {
        ...
        if (files instanceof AST_Toplevel) {
            ...
            source_maps = source_map_content && new Map();
            for (var name in files) if (HOP(files, name)) {
                ...
                if (source_map_content) {
                    if (options.sourceMap.content == "inline") {
                        ...
                        if (inlined_content) {
                            source_maps.set(name, parse_source_map(inlined_content));
                        }
                    } else if (source_map_content == "auto") {
                        var content = options.sourceMap.contents[name];
                        if (content) {
                            source_maps.set(name, parse_source_map(content));
                        }
                    } else {
                        source_maps.set(name, source_map_content);
                    }
                }
            }
